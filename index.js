...
function findElementGreaterThan(arr, num) {
  ...
  const bigNum = arr.find((elem) => elem > num);
  return bigNum || `Числа яке задовільняе умову не знайдено`;
}
...
function findIndexByProperty(arr, name) {
  ...
  const foundElement = arr.findIndex((obj) => obj.name === name);
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  const restArr = arr.slice(startIdx);
  const subArrayString = restArr.join(` `);
...
function getPositiveArrayValues(arr) {
  ...
  const positiveArrayValues = newArr.filter((elem) => elem > 0);
...
function sumAndPrint(arr) {
  ...
  const sumArr = arr.reduce((sum, currentValue) => sum + currentValue, 0);
  const printArr = arr.forEach((elem) => console.log(elem));
  return sumArr;
}
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const doubleArr = oneArr.map((x) => x ** 2);
  const filteredSquaredArray = doubleArr.filter((elem) => elem > threshold);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const cutArr = arr.slice(-n);
  const sumArr = cutArr.reduceRight(
    (sum, currentValue) => sum + currentValue,
    0
  );
  const madeArr = cutArr.forEach((elem) => console.log(elem));
  return sumArr;
}
...
function insertElementAt(arr, index, element) {
  ...
  const newArr = arr.splice(index, 0, element);
  return arr;
}
...
function findLastNegativeNumberIndex(arr) {
  ...
  const foundLastElemIn = arr.findLastIndex((num) => num < 0);
  return foundLastElemIn;
}
